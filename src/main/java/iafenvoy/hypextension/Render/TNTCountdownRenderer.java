...
public class TNTCountdownRenderer {
  public static void render(MatrixStack matrices, float partialTicks, Camera camera, Matrix4f projection,
    ...
    final Entity cameraEntity = camera.getFocusedEntity() != null ? camera.getFocusedEntity() : mc.player;
    // possible fix for optifine (see https://github.com/UpcraftLP/Orderly/issues/3)
    ...
    StreamSupport.stream(mc.world.getEntities().spliterator(), false)
        .filter(entity -> entity instanceof TntEntity && entity != cameraEntity && entity.isAlive()
            && Iterables.isEmpty(entity.getPassengersDeep())
        ...
        .map(TntEntity.class::cast).forEach(entity -> {
          renderCountdown(entity, matrices, partialTicks, camera, cameraEntity, ((TNTEntityMixin) entity).getCurrentFuseTime());
        });
  }

  private static void renderCountdown(Entity passedEntity, MatrixStack matrices, float partialTicks, Camera camera,
      Entity viewPoint, int fuse) {
    MinecraftClient mc = MinecraftClient.getInstance();
    ...
    matrices.translate(x - renderManager.camera.getPos().x,
        y - renderManager.camera.getPos().y + passedEntity.getHeight() + 0.5F, z - renderManager.camera.getPos().z);
