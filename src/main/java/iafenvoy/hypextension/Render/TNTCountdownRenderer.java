...
public class TNTCountdownRenderer {
    ...
    public static void render(MatrixStack matrices, float partialTicks, Camera camera, Matrix4f projection, Frustum capturedFrustum) {
        ...
        StreamSupport.stream(mc.world.getEntities().spliterator(), false)
                ...
                .map(TntEntity.class::cast).forEach(entity -> renderCountdown(entity, matrices, partialTicks, camera, ((TNTEntityMixin) entity).getCurrentFuseTime()));
    }

    private static void renderCountdown(Entity passedEntity, MatrixStack matrices, float partialTicks, Camera camera, int fuse) {
        matrices.push();
